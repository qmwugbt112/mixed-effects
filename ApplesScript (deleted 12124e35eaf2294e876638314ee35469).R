...
# You do not have to understand the code in this section but
# it is here in case you are keen
...
growMyApples<-function(	ntrees=3,
					nApples=N.apples
					treeMeans=c(20,40,70),
					treeSDs=c(7,20,30)){

apples<-rnorm(	N.apples*ntrees,
			mean=rep(treeMeans,each=nApples),
			sd=rep(treeSDs,each=nApples))
treeF<-factor(rep(1:ntrees,each=nApples))
